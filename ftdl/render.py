...
def render_tex(diagram_data, config):
    ...
    for node in nodes:
        lines.append("\\coordinate (%s) at (%.6f,%.6f);" % (
            node["id"], node["x"], node["y"]))

    for edge in edges:
        style = _edge_style_tex(edge)
        if style:
            lines.append("\\draw[%s] (%s) -- (%s);" % (
                style, edge["from"], edge["to"]))
        else:
            lines.append("\\draw (%s) -- (%s);" % (edge["from"], edge["to"]))
    ...
    if tex_cfg.get("show_diagram_id", True):
...
def render_svg(diagram_data, config):
    ...
    for edge in edges:
        ...
        if line_type == "wavy":
            ...
            out.append('<path d="{0}" fill="none" stroke="black" stroke-width="{1}"/>'.format(
                d, stroke_width))
